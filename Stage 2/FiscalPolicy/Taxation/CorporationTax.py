...
class FiscalCorporationTax(PRD.policies_everything):  #Inherit the policies_everything class so that it can use the LoadRatings method etc...

    def __init__(self, choice, UserID, CorporationTax, maximumslidervalue):
        
        self.UserID = UserID
        self.choice = choice
        self.nameofpolicy = "Corporation Tax"
        self.CorpT = CorporationTax
    ...
    def display_all(self):
        ...
        self.policy_label()
        self.helpbutton()
        self.slider(self.CorpT, self.maxslidervalue, 0.1, 10)   #Pass the IncomeT value for that user through to the abstract class which will the get the value it currently is and set the slider to start there
...
#-----------------------These are the methods I have overidden from the parent class----------------------------------------------------------------------------------------
    ...
    def load_help(self, help_text):
        ...
        help_text = """Progressive Tax:    Corporation Tax is a
tax placed on business profits. This will mean that the higher the
level, the unhappier businesses will become. This could have
several knock-on effects, such as increased unemployment.
However, it takes up a large proportion of the UK's tax revenue.""" #EXPLAIN MORE ABOUT IT HEREE

        
        super(FiscalCorporationTax, self).load_help(help_text) #This allows us to call the parent class method load_help which will create the window AND THEN will it be overidden
    ...
    def confirm_changes(self, slider):
        newvalue = slider.get()
        self.CheckConfirmChanges(newvalue, self.CorpT)
    ...
    def view_expectedchanges(self, slider):
        ...
        newvalue = slider.get()
        changedamount = newvalue - self.CorpT
        #print(changedamount)
        if changedamount != 0:
            ...
            self.__init__(self.nameofpolicy, self.UserID, self.CorpT, self.maxslidervalue)    #   as this would not work properly with the slider after.

        
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#End of file: CorporationTax
...
def mainfiscal():
    ...
    UserID = 1
    runfiscal = FiscalCorporationTax("Corporation Tax", UserID, 22, 100)
