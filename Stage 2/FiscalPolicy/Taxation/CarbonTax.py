...
class FiscalCarbonTax(PRD.policies_everything):  #Inherit the policies_everything class so that it can use the LoadRatings method etc...

    def __init__(self, choice, UserID, CarbonTax, maximumslidervalue):
        
        self.UserID = UserID
        self.choice = choice
        self.nameofpolicy = "Carbon Tax"
        self.CarbonT = CarbonTax
        self.maxslidervalue = maximumslidervalue
    
    
       
        super().__init__(self.nameofpolicy, self.choice, self.UserID)  #Inherit the constructer from the class policies_ratings so that the window can be displayed on the
    ...
    def display_all(self):
        ...
        self.policy_label()
        self.helpbutton()
        self.slider(self.CarbonT, self.maxslidervalue, 0.1, 10)   #Pass the IncomeT value for that user through to the abstract class which will the get the value it currently is and set the slider to start there
...
#-----------------------These are the methods I have overidden from the parent class----------------------------------------------------------------------------------------
    ...
    def load_help(self, help_text):
        ...
        help_text = """Regressive Tax:    Carbon Tax
is quite simply a tax on a unit amount of carbon. This is often used
in tradable permits to prevent businesses from exploiting
their resources and damaging the environment. Although it does
not provide a large amount of tax revenue, it is still
used often to generate revenue without effecting the economy
too much.""" #EXPLAIN MORE ABOUT IT HEREE

        
        super(FiscalCarbonTax, self).load_help(help_text) #This allows us to call the parent class method load_help which will create the window AND THEN will it be overidden
    ...
    def confirm_changes(self, slider):
        newvalue = slider.get()
        self.CheckConfirmChanges(newvalue, self.CarbonT)
    ...
    def view_expectedchanges(self, slider):
        ...
        newvalue = slider.get()
        changedamount = newvalue - self.CarbonT
        #print(changedamount)
        if changedamount != 0:
            ...
            self.__init__(self.nameofpolicy, self.UserID, self.CarbonT, self.maxslidervalue)    #   as this would not work properly with the slider after.

#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#End of file: CarbonTax
...
def mainfiscal():
    ...
    UserID = 1
    runfiscal = FiscalCarbonTax("Carbon Tax", UserID, 24, 100)
