...
class FiscalVAT(PRD.policies_everything):  #Inherit the policies_everything class so that it can use the LoadRatings method etc...

    def __init__(self, choice, UserID, VAT, maximumslidervalue):
        
        self.UserID = UserID
        self.choice = choice
        self.nameofpolicy = "Value Added Tax"
        self.VAT = VAT
    ...
    def display_all(self):
        ...
        self.policy_label()
        self.helpbutton()
        self.slider(self.VAT, self.maxslidervalue, 0.1, 10)   #Pass the IncomeT value for that user through to the abstract class which will the get the value it currently is and set the slider to start there
...
#-----------------------These are the methods I have overidden from the parent class----------------------------------------------------------------------------------------
    ...
    def load_help(self, help_text):
        ...
        help_text = """Regressive Tax:    Value Added Tax (VAT)
is a well-known tax which takes up a large chunk of the UK's total
tax revenue. It is a tax placed on all goods or services.
This often will effect those on lower incomes more.""" #EXPLAIN MORE ABOUT IT HEREE

        
        super(FiscalVAT, self).load_help(help_text) #This allows us to call the parent class method load_help which will create the window AND THEN will it be overidden
    ...
    def confirm_changes(self, slider):
        newvalue = slider.get()
        self.CheckConfirmChanges(newvalue, self.VAT)
    ...
    def view_expectedchanges(self, slider):
        ...
        newvalue = slider.get()
        changedamount = newvalue - self.VAT
        #print(changedamount)
        if changedamount != 0:
            ...
            self.__init__(self.nameofpolicy, self.UserID, self.VAT, self.maxslidervalue)    #   as this would not work properly with the slider after.
...
def mainfiscal():
    ...
    UserID = 1
    runfiscal = FiscalVAT("Value Added Tax", UserID, 22, 100)
