...
class FiscalDefence(PRD.policies_everything, OS.ExtraStatements):  #Inherit the policies_everything class so that it can use the LoadRatings method etc...

    def __init__(self, choice, UserID, Defence, maximumslidervalue):
        
        self.UserID = UserID
        self.choice = choice
        self.nameofpolicy = "Defence"
        self.Defence = Defence
        self.original = Defence
    ...
    def display_all(self):
        ...
        self.policy_label()
        self.helpbutton()
        self.slider(self.Defence, self.maxslidervalue, 1, self.tick)   #Pass the IncomeT value for that user through to the abstract class which will the get the value it
...
#-----------------------These are the methods I have overidden from the parent class----------------------------------------------------------------------------------------
    ...
    def load_help(self, help_text):
        ...
        help_text = """Defence is often forgotten about in macro-economics.
It does not have very large effects but is quite a safe option
for slowly improving your economy/developing your economy without
there being large effects you may be unaware about.""" #EXPLAIN MORE ABOUT IT HEREE

        
        super(FiscalDefence, self).load_help(help_text) #This allows us to call the parent class method load_help which will create the window AND THEN will it be overidden
    ...
    def view_expectedchanges(self, slider):
        ...
        if changedamount != 0:
            ...
            self.__init__(self.nameofpolicy, self.UserID, self.Defence, self.maxslidervalue)    #   as this would not work properly with the slider after.
...
def mainmonetary():
    ...
    UserID = 1
    runmonetary = FiscalDefence("Defence", UserID, 1, 200)
