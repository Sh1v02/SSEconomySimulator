...
class FiscalHealth(PRD.policies_everything, OS.ExtraStatements):  #Inherit the policies_everything class so that it can use the LoadRatings method etc...

    def __init__(self, choice, UserID, Health, maximumslidervalue):
        
        self.UserID = UserID
        self.choice = choice
        self.nameofpolicy = "Health"
        self.Health = Health
        self.original = Health
        self.maxslidervalue = maximumslidervalue
    

        self.value = self.getGDP()   #Inherited method
    ...
    def display_all(self):
        ...
        self.policy_label()
        self.helpbutton()
        self.slider(self.Health, self.maxslidervalue, 1, self.tick)   #Pass the IncomeT value for that user through to the abstract class which will the get the value it
...
#-----------------------These are the methods I have overidden from the parent class----------------------------------------------------------------------------------------
    ...
    def load_help(self, help_text):
        ...
        help_text = """Health is arguably one of the most important
sectors to spend on. The NHS has had very high amounts of money
injected into it. The increased spending on Health can imporove
some groups in your economy and will often lead to higher
productivity and output in your economy, but can cause your
national debt to rise at an almost proportional rate to the
benefits it creates.""" #EXPLAIN MORE ABOUT IT HEREE

        
        super(FiscalHealth, self).load_help(help_text) #This allows us to call the parent class method load_help which will create the window AND THEN will it be overidden
    ...
    def view_expectedchanges(self, slider):
        ...
        if changedamount != 0:
            ...
            self.__init__(self.nameofpolicy, self.UserID, self.Health, self.maxslidervalue)    #   as this would not work properly with the slider after.
...
def mainmonetary():
    ...
    UserID = 1
    runmonetary = FiscalHealth("Health", UserID, 12, 200)
