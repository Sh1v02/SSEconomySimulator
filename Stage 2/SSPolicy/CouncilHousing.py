...
class SSCouncilHousing(PRD.policies_everything, OS.ExtraStatements):  #Inherit the policies_everything class so that it can use the LoadRatings method etc...

    def __init__(self, choice, UserID, CouncilHousing, maximumslidervalue):
        
        self.UserID = UserID
        self.choice = choice
        self.nameofpolicy = "Council Housing"
        self.CouncilH = CouncilHousing
        self.original = CouncilHousing
    ...
    def display_all(self):
        ...
        self.policy_label()
        self.helpbutton()
        self.slider(self.CouncilH, self.maxslidervalue, 1, self.tick)   #Pass the IncomeT value for that user through to the abstract class which will the get the value it currently is and set the slider to start there
...
#-----------------------These are the methods I have overidden from the parent class----------------------------------------------------------------------------------------
    ...
    def load_help(self, help_text):
        ...
        help_text = """Council Housing is spending focussed on
building more council houses. This will give more people
a place to live in and will improve their standard of living.""" #EXPLAIN MORE ABOUT IT HEREE

        
        super(SSCouncilHousing, self).load_help(help_text) #This allows us to call the parent class method load_help which will create the window AND THEN will it be overidden
    ...
    def view_expectedchanges(self, slider):
        ...
        if changedamount != 0:
            ...
            self.__init__(self.nameofpolicy, self.UserID, self.CouncilH, self.maxslidervalue)    #   as this would not work properly with the slider after.
                

        
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#End of file: CouncilHousing
...
def mainmonetary():
    ...
    UserID = 1
    runmonetary = SSCouncilHousing("Council Housing", UserID, 12, 200)
