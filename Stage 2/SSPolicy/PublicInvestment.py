...
class SSPublicInvestment(PRD.policies_everything, OS.ExtraStatements):  #Inherit the policies_everything class so that it can use the LoadRatings method etc...

    def __init__(self, choice, UserID, PublicInvestment, maximumslidervalue):
        
        self.UserID = UserID
        self.choice = choice
        self.nameofpolicy = "Public Investment"
        self.PublicInvestment = PublicInvestment
        self.original = PublicInvestment
    ...
    def display_all(self):
        ...
        self.policy_label()
        self.helpbutton()
        self.slider(self.PublicInvestment, self.maxslidervalue, 1, self.tick)   #Pass the IncomeT value for that user through to the abstract class which will the get the value it currently is and set the slider to start there
...
#-----------------------These are the methods I have overidden from the parent class----------------------------------------------------------------------------------------
    ...
    def load_help(self, help_text):
        ...
        help_text = """Public Sector Investment is spending on the economy,
such as spending on infrastructure like roads and bridges. This can improve
your economies productivity and therefore output, while making
businesses lives easier as transportation becomes easier.""" #EXPLAIN MORE ABOUT IT HEREE

        
        super(SSPublicInvestment, self).load_help(help_text) #This allows us to call the parent class method load_help which will create the window AND THEN will it be overidden
    ...
    def view_expectedchanges(self, slider):
        ...
        if changedamount != 0:
            ...
            self.__init__(self.nameofpolicy, self.UserID, self.PublicInvestment, self.maxslidervalue)    #   as this would not work properly with the slider after.
                
        
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#End of file: PublicInvestment
...
def mainmonetary():
    ...
    UserID = 1
    runmonetary = SSPublicInvestment("Public Sector Investment", UserID, 12, 200)
